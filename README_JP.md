# HomesPlugin
## サポートを受ける [![Discord](https://img.shields.io/badge/Discord-Join-blue?logo=discord&logoColor=white)](https://discord.gg/pkxGMSgV86)

[English](README.md) | [日本語](README_JP.md)

Minecraftサーバー（Spigot/Paper）向けの、堅牢で使いやすいホーム管理プラグインです。
このプラグインを使用すると、プレイヤーは複数のホームを設定し、GUIで管理し、さらに他のプレイヤーに公開することができます。

## 機能

- **GUIベースの管理**: チェスト形式の直感的なインターフェースでホームを管理できます。
- **複数のホーム**: プレイヤーは複数のホームを設定できます（デフォルト: 3、設定で上限変更可能）。
- **公開/非公開の切り替え**: ホームを公開して、他のプレイヤーが訪問できるように設定できます。
- **経済システムのサポート**: Vaultと連携し、ホームの設定、テレポート、公開設定の変更に費用を設定できます。
- **権限による Bypass**: OP（または LuckPerms で任意のグループ）は、テレポート待機時間と利用料金をスキップできます。
- **Vanish対応**: vanish中のプレイヤー（SuperVanish / PremiumVanish / EssentialsX / CMI）は TPA の一覧・タブ補完から除外され、対象にもできません。
- **データベース対応**: H2（ローカルファイル、デフォルト）およびMySQL/MariaDBをサポートしています。
- **タブ補完**: コマンドやプレイヤー名の入力時にスマートなタブ補完が利用できます。
- **非同期処理**: 重いデータベース操作は非同期で処理され、サーバーのラグを防ぎます。
- **ワールド別アイコン**: ワールドごとにホームのアイコン（ベッドの色など）をカスタマイズできます。
- **Folia 対応**: `folia-supported: true`。テレポート・GUI・TPA・DB は Paper のリージョンスケジューラを使います。

## コマンド

| コマンド                 | 説明                                | 権限             |
| :------------------- | :-------------------------------- | :------------- |
| `/sethome <名前>`      | 現在の位置にホームを設定します。                  | —              |
| `/delhome <名前>`      | 指定したホームを削除します。                    | —              |
| `/home <名前>`         | 指定したホームにテレポートします。                 | —              |
| `/homes`             | ホーム管理GUIを開きます。                    | —              |
| `/vhome`               | 公開ホーム一覧 GUI を開きます（持ち主の頭アイコン）。 | —              |
| `/vhome <プレイヤー>`     | 指定プレイヤーの公開ホーム一覧を開きます。（管理者は全て閲覧可能） | —              |
| `/homes reload`      | プラグインの設定をリロードします。                 | `homes.reload` |

## 権限 (Permissions)

- `homes.admin`: 他人の非公開ホームの閲覧・管理権限（デフォルト: OP）。
- `homes.reload`: プラグイン設定のリロード権限（デフォルト: OP）。
- `homes.limit.<数値>`: プレイヤーが作成できるホームの最大数（例: `homes.limit.5`）。複数持つ場合は最大値が使われます。
- `homes.limit.unlimited`: ホーム数を無制限にします（デフォルト: false）。
  - これらを付与しない場合は `settings.default-home-limit` が適用されます（OPも同様）。
- `homes.bypass.teleportdelay`: 待機時間（ウォームアップ）を無視して即テレポートします（デフォルト: OP）。
- `homes.bypass.economy`: ホーム機能を無料で使えます（全コストをスキップ／デフォルト: OP）。
- `homes.tpa.seehidden`: vanish中のプレイヤーも TPA で見え、対象にできます（デフォルト: OP）。

> いずれも LuckPerms 等で付与・剥奪できます（例: `lp user <名前> permission set homes.bypass.economy false`）。

## 設定 (Configuration)

### データベース

デフォルトではH2（ローカルファイルデータベース）を使用します。MySQL/MariaDBを使用する場合は、`config.yml`の `database.type` を変更してください。

```yaml
database:
  type: "mysql" # または "mariadb"
  host: "localhost"
  port: "3306"
  name: "minecraft"
  user: "root"
  password: "password"
```

### 経済システム (Economy)

Vaultおよび経済プラグイン（EssentialsXなど）が必要です。

```yaml
economy:
  cost:
    set-home: 100.0      # ホーム作成費用
    teleport: 10.0       # 自分のホームへのテレポート費用
    make-public: 500.0   # 公開設定費用
    visit-public: 10.0   # 他人の公開ホーム訪問費用（そのまま持ち主へ渡る）
```

### GUIのカスタマイズ

`config.yml` でGUIのタイトル、アイコン、メッセージをカスタマイズできます。

## インストール方法

1. [GitHub Releases](https://github.com/paper0319/homes-plugin/releases) から `HomesPlugin-x.y.z.jar` をダウンロードします。
2. サーバーの `plugins` フォルダに配置します。
3. （オプション）経済機能を使用する場合は、Vaultと経済プラグインをインストールします。
4. サーバーを再起動します。

## 開発者

Developed by naonao.
